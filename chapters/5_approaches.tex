...
The old behavior remains in the RCEnumerator, the new behavior is implemented in the RCSelectiveEnumerator and the RCStartEnumerator, the former being a two-stage enumerator. The initial one is the RCStartEnumerator, which only scans Type1, and the second one is the RCEnumerator, which scans by default the whole range of Type1, Type2 and Type3. The RCSelectiveEnumerator connects them with a registered connector function. It is called as soon as the first stage is done and the second is to be started and expands the found identifiers of the RCStartEnumerator to ranges and also resolves any overlaps to a continuous range. The connector then creates the configuration for the RCEnumerator of the next stage. It restricts the scan area to the identifiers just created and additionally to Type2 and Type3.
